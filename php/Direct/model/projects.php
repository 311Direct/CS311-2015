...
{    
  public function retrieveProjectWithID($id)
  {
    ...
    $rs[0]['projectManagerUserIds'] = array('yang','michael');
    ...
    $genericSearchSingle = $this->dbh->prepare("SELECT Tasks.id, priority, taskTitle AS title, Tasks.status FROM Tasks JOIN Projects ON Tasks.project_id = Projects.id JOIN Users on Tasks.assignee = Users.id WHERE Projects.id = :id");
    $genericSearchSingle->bindParam(':id', $id);
    $genericSearchSingle->execute();
    
    $rs_tasks = $genericSearchSingle->fetchAll(PDO::FETCH_ASSOC);
    
    for($i=0; $i< count($rs_tasks); $i++)
    {
    	$rs_tasks[$i]['assignee'] = [];
    	$genericSearchSingle = $this->dbh->prepare("SELECT username, displayname from Users Join TaskAssignees on Users.id = TaskAssignees.user_id Join Tasks on TaskAssignees.task_id = Tasks.id where Tasks.id = :id");
    	$genericSearchSingle->bindParam(':id', $rs_tasks[$i]['id']);
    	$genericSearchSingle->execute();
    	
    	$rs_assignee = $genericSearchSingle->fetchAll(PDO::FETCH_ASSOC);
    	
    	array_push($rs_tasks[$i]['assignee'], $rs_assignee);
  ...
  public function retrieveAllProjectInSystem()
  {
